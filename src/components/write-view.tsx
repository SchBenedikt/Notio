...
export function WriteView({ cards }: WriteViewProps) {
  ...
  const isCorrect = isAnswered && currentCard && inputValue.trim().toLowerCase() === currentCard.term.toLowerCase();
  ...
  const handleCheck = (e: React.FormEvent) => {
    ...
    setSession(prev => {
        ...
        if (inputValue.trim().toLowerCase() === currentCard.term.toLowerCase()) {
        ...
        <form onSubmit={handleCheck}>
            <div className="relative">
                <Input
                    ...
                    className={cn(
                        ...
                        isAnswered && isCorrect && "border-green-500 focus-visible:ring-green-500",
                        isAnswered && !isCorrect && "border-red-500 focus-visible:ring-red-500"
                    )}
                />
                <div className="absolute inset-y-0 right-3 flex items-center">
                    {isAnswered && isCorrect && <CheckCircle className="h-5 w-5 text-green-500" />}
                    {isAnswered && !isCorrect && <XCircle className="h-5 w-5 text-red-500" />}
                </div>
            </div>
            {!isAnswered && (
                <Button type="submit" className="w-full mt-4" disabled={!inputValue.trim()}>
                    Antwort prüfen
                </Button>
            )}
        </form>
         {isAnswered && !isCorrect && (
            <Alert variant="destructive">
                <AlertTitle>Richtige Antwort:</AlertTitle>
                <AlertDescription className="font-semibold">{currentCard?.term}</AlertDescription>
            </Alert>
        )}
      </CardContent>
      <CardFooter>
